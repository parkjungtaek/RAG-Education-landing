...
export const ModulesSection = () => {
  return (
    <section
      id="modules"
      className="relative py-32 px-4 bg-gradient-to-b from-black via-slate-950 to-black overflow-hidden"
    >
        ...
        {[...Array(50)].map((_, i) => (
          <motion.div
            ...
            style={{
              ...
              top: `${Math.random() * 100}%`,
              background: `rgba(${
                Math.random() > 0.5 ? "6,182,212" : "168,85,247"
              },${Math.random() * 0.5 + 0.3})`,
            }}
            animate={{
              y: [0, -30, 0],
      ...
      <div className="container mx-auto relative z-10">
        ...
        >
          <h2 className="text-5xl md:text-6xl font-black mb-6 text-white">학습 모듈</h2>
          <p className="text-xl md:text-2xl text-gray-400 font-medium">연결된 지식의 네트워크</p>
        </motion.div>

        {/* ====== 모듈 그래프 래퍼 (여기 좌표계 기준으로 연결선을 그릴 거라 relative 유지) ====== */}
        <div className="relative w-full h-[800px] max-w-7xl mx-auto">
          {/* (A) 기존 모듈-모듈 연결선 SVG (배경) */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 1 }}>
            <defs>
              {modules.map((module) => {
                ...
                return (
                  <linearGradient
                    key={`gradient-${module.id}`}
                    id={`lineGradient-${module.id}`}
                    x1="0%"
                    y1="0%"
                    x2="100%"
                    y2="0%"
          ...
          {modules.map((module, index) => {
            ...
            return (
              <motion.div
                key={module.id}
                id={`mod-${module.id}`}  // ✅ 모듈 고정 ID (mod-1 ~ mod-4)
                className="absolute"
                style={{
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="mb-6 relative">
                      <div
                        className={`absolute inset-0 bg-gradient-to-r ${colors.bg} rounded-2xl blur-xl opacity-60`}
                      />
                      <module.icon
                        className={`relative w-12 h-12 md:w-16 md:h-16 ${colors.text} ${colors.shadow}`}
                      />
                    </div>
                    ...
                    >
                      <span className={`text-sm font-bold ${colors.text}`}>Module_{module.id}</span>
                    </motion.div>
        ...
        {/* ✅ 선택된 프로젝트 설명 패널 */}
        <div id="proj-desc" className="mt-10 text-sm text-gray-300">
          프로젝트를 클릭하면 설명이 표시됩니다.
        </div>
